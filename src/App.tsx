...
// --- SHUTDOWN WIZARD ---
const ShutdownWizard = ({ isOpen, onClose }: any) => {
    const { tasks, saveItem } = useDataStore();
            ...
            <div className="glass-heavy max-w-lg w-full rounded-3xl p-8 relative border border-slate-700 shadow-2xl">
                ...
                {step === 1 && (
                    <div className="text-center space-y-6 py-8">
                        <div className="w-20 h-20 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto text-4xl animate-bounce">🍵</div>
                        <div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="max-h-[300px] overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                            {pendingTasks.length === 0 ? (
                                <div className="text-center py-8 text-slate-500 italic">No pending tasks due today. You are free! 🕊️</div>
                                ...
                                <div key={t.id} className="bg-slate-900/50 border border-slate-800 p-4 rounded-xl flex items-center justify-between">
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => saveItem('tasks', { ...t, dueDate: getTomorrow() })} className="px-3 py-1.5 bg-blue-600/10 text-blue-400 hover:bg-blue-600 hover:text-white text-xs font-bold rounded-lg transition-colors flex items-center gap-1">
                                            <Sun size={12} /> Tmrw
                                        </button>
                                        <button onClick={() => saveItem('tasks', { ...t, dueDate: getNextMonday() })} className="px-3 py-1.5 bg-purple-600/10 text-purple-400 hover:bg-purple-600 hover:text-white text-xs font-bold rounded-lg transition-colors flex items-center gap-1">
                        ...
                        {pendingTasks.length === 0 ? (
                            ...
                            <div className="text-center text-xs text-slate-500">{pendingTasks.length} tasks remaining</div>
                        )}
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="text-center space-y-6 py-8">
                        <div className="w-20 h-20 bg-indigo-500/20 rounded-full flex items-center justify-center mx-auto text-4xl">🌙</div>
                        <div>
...
const LoadingFallback = () => {
    const [message] = useState(() => {
        const msgs = ["Aligning the chakras... 🧘‍♀️", "Brewing digital coffee... ☕️", "Getting your flow ready... ✨", "Hello, Hanna! 👋", "Tying up loose ends... 🎀", "Checking the stars... 🌟"];
...
const LoginScreen = () => {
  ...
  useEffect(() => {
    // 1. Clear any ghost verifier from a previous session
    ...
    // 2. Initialize a fresh one attached to the NEW DOM element
    (window as any).recaptchaVerifier = new RecaptchaVerifier(auth, 'recaptcha-container', { 'size': 'invisible' });

    // 3. Clean it up when we log in and the screen unmounts!
    return () => {
      if ((window as any).recaptchaVerifier) {
         try {
            (window as any).recaptchaVerifier.clear();
  ...
  const handleSendCode = async (e: any) => {
    ...
    try {
      ...
      // If it fails, refresh the verifier so the user can try again
      if ((window as any).recaptchaVerifier) {
...
const EmojiRain = ({ isActive, onComplete }: { isActive: boolean, onComplete: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 z-[1000] pointer-events-none overflow-hidden">
            ...
            `}</style>
            {drops.map((drop) => (
                <div key={drop.id} className="absolute top-0 text-3xl" style={{ left: `${drop.left}%`, animation: `fall-and-sway ${drop.animationDuration}s linear ${drop.delay}s forwards` }}>
                    {drop.emoji}
                </div>
            ))}
        </div>
...
function App() {
  ...
  useEffect(() => {
      const handleKeyDown = (e: KeyboardEvent) => {
          if (e.altKey && e.key.toLowerCase() === 'n') { e.preventDefault(); setIsQuickTaskOpen(true); }
          if (e.key === 'Escape') { setIsQuickTaskOpen(false); }
      };
  ...
  // 3. Central Sync Listener 
  useEffect(() => {
      if (!store.user) return;
      const unsubscribe = store.subscribeToData(store.user.uid);
      return () => unsubscribe();
  }, [store.user?.uid]);

  // 4. The Boot Sequence (Morning Reset)
  useEffect(() => {
      if (!store.user || store.tasks.length === 0) return;
  ...
  }, [store.user, store.tasks]);

  // --- DASHBOARD HELPERS ---
  const getDashboardData = () => {
      const today = getToday();
      const todaysTasks = store.tasks.filter((t:any) => {
          if (t.archived || t.status === 'Done') return false;
          return t.dueDate === today || t.dueDate < today;
      }).sort((a:any, b:any) => a.dueDate.localeCompare(b.dueDate));

      const recentFiles = [
          ...store.docs.map((d:any) => ({ ...d, type: 'doc', icon: FileText })),
          ...store.whiteboards.map((w:any) => ({ ...w, type: 'board', icon: PenTool }))
      ].sort((a:any, b:any) => {
  ...
  if (store.loading) return <div className="h-screen bg-slate-950 flex"><LoadingFallback /></div>;
      ...
      <main className="flex-1 ml-64 flex flex-col h-screen relative z-10 overflow-hidden">
                ...
                <input 
                    ...
                    onKeyDown={(e: any) => {
                        if (e.key === 'Enter' && e.target.value.trim()) {
                            // DIRECT STORE SAVE
                            store.saveItem('tasks', { title: e.target.value.trim(), ice: 5, dueDate: getToday(), status: 'Not Started', archived: false });
                            e.target.value = '';
        ...
        <div className="flex-1 overflow-y-auto p-8 relative">
          <Suspense fallback={<LoadingFallback />}>
                    ...
                    <div className="flex justify-between items-end border-b border-slate-800 pb-6">
                        <div>
                            <div className="flex items-center gap-4 mb-2">
                                ...
                                <button onClick={() => setIsRaining(true)} disabled={isRaining} className="p-2 rounded-full bg-pink-500/10 text-pink-400 hover:bg-pink-500 hover:text-white disabled:opacity-50 transition-all hover:scale-110 active:scale-95 shadow-lg shadow-pink-500/10" title="Make it rain vibes">✨</button>
                            </div>
                        ...
                        <div className="flex gap-4 text-sm text-slate-500">
                             <span>{store.tasks.filter((t:any) => !t.archived && t.status !== 'Done').length} Pending Tasks</span>
                             <span>•</span>
                             <span>{store.projects.filter((p:any) => p.status !== 'Done').length} Active Projects</span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <h3 className="font-bold text-slate-200 uppercase text-sm tracking-wider">Priority Tasks</h3>
                            </div>
                            ...
                            {todaysTasks.length === 0 ? (
                                ...
                                <div className="space-y-3">
                                    {todaysTasks.map((t:any) => {
                                        ...
                                        return (
                                            <div key={t.id} onClick={() => { setFocusTaskId(t.id); setActiveTab('tasks'); }} className={`glass p-4 rounded-xl cursor-pointer group hover:bg-slate-800 transition-all border-l-4 ${isOverdue ? 'border-red-500' : 'border-blue-500'}`}>
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <h4 className="font-bold text-slate-200 group-hover:text-white">{t.title}</h4>
                                                        <div className="flex items-center gap-2 mt-1 text-xs text-slate-500">
                                                            {t.project && <span className="bg-slate-800 px-2 py-0.5 rounded text-slate-400">{store.projects.find((p:any) => p.id === t.project)?.name}</span>}
                                                            <span className={`${isOverdue ? 'text-red-400 font-bold' : ''}`}>{isOverdue ? 'Overdue (' + t.dueDate + ')' : 'Due Today'}</span>
                                                        </div>
                                                    </div>
                            ...
                            {recentFiles.length === 0 ? (
                                ...
                                <div className="glass rounded-2xl overflow-hidden">
                                    {recentFiles.map((item:any, i:number) => (
                                        <div key={item.id} onClick={() => setActiveTab(item.type === 'doc' ? 'docs' : 'whiteboard')} className={`p-4 flex items-center justify-between cursor-pointer hover:bg-slate-800/50 transition-colors ${i !== recentFiles.length - 1 ? 'border-b border-slate-800/50' : ''}`}>
            ...
            {activeTab === 'tasks' && <TaskManager focusTaskId={focusTaskId} clearFocus={() => setFocusTaskId(null)} />}
            {activeTab === 'projects' && <ProjectsHub onNavigateToDoc={(id: string) => { setFocusDocId(id); setActiveTab('docs'); }} onNavigateToBoard={(id: string) => { setFocusBoardId(id); setActiveTab('whiteboard'); }} />}
            {activeTab === 'docs' && <DocsModule focusDocId={focusDocId} clearFocus={() => setFocusDocId(null)} />}
            {activeTab === 'whiteboard' && <WhiteboardModule focusBoardId={focusBoardId} clearFocus={() => setFocusBoardId(null)} />}
        ...
        <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-[20vh] bg-slate-950/80 backdrop-blur-md p-4">
            <div className="w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden animate-[fadeIn_0.1s_ease-out]">
                <div className="flex items-center px-4 py-3 bg-slate-800/50 border-b border-slate-700">
                    ...
                    <input autoFocus type="text" placeholder="Quick Add Task... (Press Enter to save)" className="w-full bg-transparent text-lg text-white placeholder-slate-500 focus:outline-none" onKeyDown={(e: any) => {
                            if (e.key === 'Enter' && e.target.value.trim()) {
                                store.saveItem('tasks', { title: e.target.value.trim(), ice: 5, dueDate: getToday(), status: 'Not Started', archived: false });
                    ...
                    />
                    <button onClick={() => setIsQuickTaskOpen(false)} className="text-slate-500 hover:text-white ml-2 shrink-0"><X size={18} /></button>
                </div>
            </div>
        </div>
      )}

      <ShutdownWizard isOpen={isShutdownOpen} onClose={() => setIsShutdownOpen(false)} />
