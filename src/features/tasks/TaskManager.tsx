...
export const TaskManager = ({ focusTaskId, clearFocus }: any) => {
  ...
  useEffect(() => {
    if (focusTaskId) {
      const task = tasks.find((t: any) => t.id === focusTaskId);
      if (task) {
        setEditingTask({ ...task });
        setModalTab('details');
        setIsModalOpen(true);
  ...
  const saveTask = () => {
    ...
    // --- HISTORY LOGIC ---
    const original = tasks.find((t: any) => t.id === editingTask.id);
  ...
  const selectedProjectName = projects.find((p:any) => p.id === editingTask?.project)?.name || "-- No Project --";
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      ...
      <div className="flex-1 bg-slate-900/50 border border-slate-800/50 rounded-2xl overflow-hidden flex flex-col">
                            ...
                            <td className="p-5">
                                {t.project ? (
                                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20">
                                        {projects.find((p:any) => p.id === t.project)?.name || 'Unknown'}
                                    </span>
                                ) : (
                            ...
                            <td className="p-5 text-right">
                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button 
                                        onClick={(e) => { 
                                            e.stopPropagation(); 
                                            saveItem('tasks', { ...t, dueDate: addDays(getToday(), 1) });
                                        }} 
        ...
        {/* --- PAGINATION FOOTER --- */}
        {sortedAndFiltered.length > ITEMS_PER_PAGE && (
            <div className="border-t border-slate-800/50 p-4 flex items-center justify-between bg-slate-900/30">
                <div className="text-xs text-slate-500">
                    Showing {startIndex + 1}-{Math.min(startIndex + ITEMS_PER_PAGE, sortedAndFiltered.length)} of {sortedAndFiltered.length} tasks
                </div>
                <div className="flex items-center gap-2">
                    <button 
